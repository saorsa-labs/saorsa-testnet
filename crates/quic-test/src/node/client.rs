...
const VPS_NODE_IPS: &[&str] = &[
    "77.42.75.115",    // saorsa-1 (registry)
    ...
    "77.42.39.239",    // saorsa-10 (Symmetric NAT test node)
];
...
impl TestNode {
    pub async fn new(
        ...
        // In local_only mode, skip external VPS bootstrap to avoid NAT traversal issues in Docker
        let vps_bootstrap = if config.local_only {
            info!("Local-only mode: skipping external VPS bootstrap peers");
            Vec::new()
        } else {
            vps_gossip_bootstrap_addrs()
        };
        ...
        // For comprehensive peer cache sharing - include ALL known peers, not just connected
        let gossip_integration_for_events = Arc::clone(&gossip_integration);
        let epidemic_gossip_for_events = Arc::clone(&epidemic_gossip);
                    ...
                    P2pEvent::PeerConnected {
                        ...
                        // comprehensive test which has more detailed info.
                        {
                            ...
                            if !peers.contains_key(&peer_hex) {
                                ...
                                // CRITICAL: Add outbound peer to HyParView membership
                                // Inbound peers get add_peer() in a separate handler, but outbound
                                // connections MUST also be added or gossip protocols won't see them!
                                if !is_inbound {
                                    if let Ok(peer_bytes) = hex::decode(&peer_hex) {
                                        if peer_bytes.len() == 32 {
                                                ...
                                                .await
                                            {
                                                ...
                                                info!(
                                                    "SUCCESS: Added outbound peer {} to HyParView active view",
                                                    &peer_hex[..8.min(peer_hex.len())]
                                                );
                                            }
                                        }
                        ...
                        // When we connect to a peer, we send them our list of ALL known peers,
                        // not just currently connected ones - this is key for epidemic spread.
                        let node_for_gossip = Arc::clone(&node_for_events);
                        let connected_peers_for_gossip = Arc::clone(&connected_peers_for_events);
                        let gossip_integration_for_gossip =
                            Arc::clone(&gossip_integration_for_events);
                        let epidemic_gossip_for_gossip = Arc::clone(&epidemic_gossip_for_events);
                        ...
                        tokio::spawn(async move {
                            ...
                            // Build comprehensive peer list from ALL sources:
                            // 1. Currently connected peers
                            // 2. Peers from GossipIntegration cache
                            // 3. Peers from EpidemicGossip CRDT cache
                            let peer_list = build_comprehensive_peer_list(
                                &connected_peers_for_gossip,
                                &gossip_integration_for_gossip,
                                &epidemic_gossip_for_gossip,
                            ...
                            .await
                            {
                                ...
                                info!(
                                    "Sent comprehensive peer list ({} peers) to newly connected {}",
                                    peer_list.len(),
                                    &new_peer_hex[..8.min(new_peer_hex.len())]
                                );
                            }
                                ...
                                EpidemicEvent::PeerJoined { peer_id, addresses } => {
                                    ...
                                    // PRESENCE: Add peer to CRDT and broadcast join to groups
                                    {
                                        ...
                                        if let Ok(payload) = serde_json::to_vec(&presence) {
                                            let topic_id = saorsa_gossip_types::TopicId::from_entity("nat-events");
    ...
    async fn discover_external_address(&self) {
        ...
        if self.config.local_only {
            info!("Local-only mode: skipping external address discovery via VPS nodes");
            return;
        }
            ...
            Ok(response) => {
                if response.success {
                    ...
                    if !response.peers.is_empty() {
                        ...
                        // Filter to only include known VPS node addresses on port 9000
                        // This prevents adding unreachable NAT'd addresses with ephemeral ports
                        let bootstrap_addrs: Vec<SocketAddr> = response
                            ...
                            .filter(|addr| is_vps_node(addr) && addr.port() == 9000)
                            .collect();
    ...
    fn spawn_heartbeat_loop(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            while !shutdown.load(Ordering::SeqCst) {
                ...
                // Use non-blocking try_read() first, fall back to async with short timeout
                // This prevents heartbeat failures when stats_updater holds write lock
                let epidemic_stats = if let Some(stats) = epidemic_gossip.stats_nonblocking() {
                    stats
                } else {
                    // Non-blocking failed, try async with short timeout
                    match tokio::time::timeout(Duration::from_millis(500), epidemic_gossip.stats())
                        .await
                    {
                        Ok(stats) => stats,
                        Err(_) => {
                            // Lock contention - use default stats to keep heartbeat alive
                            debug!(
                                "Heartbeat #{}: stats lock contended, using defaults",
                                heartbeat_count
                            );
                            GossipStats::default()
                        }
                ...
                let (conn_direct_ipv4, conn_direct_ipv6, conn_hole_punched, conn_relayed) = {
                    ...
                    if hb_count % 10 == 0 {
                        let p2p_total = peer_count + stats.inbound_connections as usize;
                ...
                // peer_count = outbound connections we initiated
                // stats.inbound_connections = connections initiated by others
                let total_connections = peer_count + stats.inbound_connections as usize;
    ...
    /// Spawn a background task that runs proof verification every 60 seconds.
    /// Results are sent to the TUI via TuiEvent::ProofStatusUpdate.
    fn spawn_proof_orchestrator_loop(&self) -> tokio::task::JoinHandle<()> {
        use crate::proof_orchestrator::{ProofOrchestrator, ProofOrchestratorConfig};
        use crate::tui::ProofStatus;
        ...
        tokio::spawn(async move {
            // Wait 30 seconds before first run to let network stabilize
            tokio::time::sleep(Duration::from_secs(30)).await;
            ...
            while !shutdown.load(Ordering::SeqCst) {
                ...
                // Register ourselves
                orchestrator.register_node(peer_id.clone());

                // Collect connected peers and their gossip stats
                let peers = connected_peers.read().await;
                let mut total_nodes = 1usize; // ourselves
                let mut direct_count = 0usize;
                let mut holepunched_count = 0usize;
                let mut relayed_count = 0usize;

                for (peer_hex, info) in peers.iter() {
                    total_nodes += 1;
                    orchestrator.register_node(peer_hex.clone());
                    orchestrator.record_connections(&peer_id, vec![peer_hex.clone()]);

                    match info.method {
                        crate::registry::ConnectionMethod::Direct => direct_count += 1,
                        crate::registry::ConnectionMethod::HolePunched => holepunched_count += 1,
                        crate::registry::ConnectionMethod::Relayed => relayed_count += 1,
                    }
                }
                drop(peers);

                // Get gossip stats from epidemic gossip
                let gossip_stats = epidemic_gossip.stats().await;

                // Record our own gossip stats
                    ...
                    .await
                {
                    for peer in &registry_peers {
                        if peer.peer_id != peer_id {
                            ...
                            // Record peer's gossip stats if available
                            if let Some(ref node_stats) = peer.gossip_stats {
                                use crate::epidemic_gossip::{
                                    ConnectionBreakdown, CoordinatorStats, CrdtStats, GossipStats,
                                    GroupStats, HyParViewStats, PlumtreeStats, RendezvousStats,
                                    SwimStats,
                                };
                                let peer_gossip = GossipStats {
                                ...
                                orchestrator.record_gossip_stats(&peer.peer_id, peer_gossip);
                            }
                        }
                    }
                    total_nodes = registry_peers.len().max(total_nodes);
                }

                // Run comprehensive proof test
                let report = orchestrator.run_comprehensive_test();

                // Convert to ProofStatus for TUI
                let mut status = ProofStatus::new();
                ...
                // Gossip
                status.gossip_pass = report
                    .step_results
                    .iter()
                    .find(|r| r.name == "gossip_protocols")
                    .map(|r| r.passed)
                    .unwrap_or(false);
                ...
                status.crdt_pass = report
                    .step_results
                    .iter()
                    .find(|r| r.name == "crdt_convergence")
                    .map(|r| r.passed)
                    .unwrap_or(false);
                status.crdt_nodes = total_nodes;
                status.crdt_hash_short = None; // Could extract from report if available

                // NAT
                status.nat_pass = direct_count + holepunched_count + relayed_count > 0;
                status.nat_direct = direct_count;
                status.nat_punched = holepunched_count;
                status.nat_relayed = relayed_count;

                // Send update to TUI
                let _ = event_tx.try_send(TuiEvent::ProofStatusUpdate(status));

                debug!(
                    "Proof orchestrator run complete: {} (session {})",
...
/// Build a comprehensive list of ALL known peers for gossip exchange.
///
/// This combines peers from multiple sources:
/// 1. Currently connected peers (highest priority - we know they're alive)
/// 2. Peers from GossipIntegration cache (bootstrap cache, received announcements)
/// 3. Peers from EpidemicGossip CRDT peer cache (distributed via epidemic gossip)
///
/// This is critical for epidemic spread of peer information - we share ALL known
/// peers, not just ones we're currently connected to, so clients behind NAT can
/// discover the full network without relying on the registry.
async fn build_comprehensive_peer_list(
    connected_peers: &Arc<RwLock<HashMap<String, TrackedPeer>>>,
    gossip_integration: &Arc<GossipIntegration>,
    epidemic_gossip: &Arc<EpidemicGossip>,
    our_peer_id: &str,
    ...
    // Use a HashMap to deduplicate by peer_id, keeping the most recent info
    let mut peer_map: HashMap<String, GossipPeerInfo> = HashMap::new();
    let mut seen_ids: HashSet<String> = HashSet::new();

    // 1. First, add all connected peers (highest priority - we know they're alive)
    {
        let connected = connected_peers.read().await;
        for (peer_id, tracked) in connected.iter() {
    ...
    // 2. Add peers from GossipIntegration (via GossipDiscovery's known peers)
    let gossip_peers = gossip_integration.discovery().get_peers().await;
    for announcement in gossip_peers {
        if announcement.peer_id == our_peer_id {
            continue;
        }
        if !seen_ids.contains(&announcement.peer_id) {
            seen_ids.insert(announcement.peer_id.clone());
            peer_map.insert(
                ...
                GossipPeerInfo {
                    ...
                    addresses: announcement.addresses.clone(),
                    is_public: announcement.is_public,
                    is_connected: false, // Not currently connected
                    last_seen_ms: announcement.timestamp_ms,
                },
            );
        }
    }

    // 3. Add peers from EpidemicGossip CRDT peer cache
    let crdt_entries = epidemic_gossip.get_peer_cache_entries().await;
    for entry in crdt_entries {
        let peer_id_hex = hex::encode(entry.peer_id);
        if peer_id_hex == our_peer_id {
            continue;
        }
        if !seen_ids.contains(&peer_id_hex) {
            seen_ids.insert(peer_id_hex.clone());
            peer_map.insert(
                peer_id_hex.clone(),
                GossipPeerInfo {
                    peer_id: peer_id_hex,
                    addresses: entry.addresses.clone(),
                    is_public: false, // CRDT cache doesn't track public status
                    is_connected: false,
    ...
    // 4. Add peers from GossipIntegration's bootstrap cache (persistent peer cache)
    // Get up to 100 peers from the bootstrap cache to share
    let cached_peers = gossip_integration.get_best_cached_peers(100);
    for entry in cached_peers {
        let peer_id_hex = hex::encode(entry.peer_id.as_bytes());
        if peer_id_hex == our_peer_id {
            continue;
        }
        if !seen_ids.contains(&peer_id_hex) {
            seen_ids.insert(peer_id_hex.clone());
            // Check if peer is public based on NAT class (Eim = Endpoint-independent mapping = public)
            let is_public = matches!(entry.nat_class, saorsa_gossip_coordinator::NatClass::Eim);
            // Combine public and reflexive addresses
            let mut addresses = entry.public_addrs.clone();
            addresses.extend(entry.reflexive_addrs.iter().cloned());
            if addresses.is_empty() {
                continue; // Skip peers with no addresses
            }
            peer_map.insert(
                peer_id_hex.clone(),
                GossipPeerInfo {
                    ...
                    is_public,
                    is_connected: false,
                    last_seen_ms: now_ms, // Use current time since cache doesn't track last_seen
                },
            );
        }
    }
