...
impl PeerStore {
    ...
    // =========================================================================
    // Proof-Based Testing Methods
    // =========================================================================

    /// Build a connectivity proof from the current peer store state.
    ///
    ...
    pub fn build_connectivity_proof(&self, observer_id: &str) -> super::types::NetworkConnectivityProof {
        ...
        let mut proof = NetworkConnectivityProof::new(observer_id.to_string());

        // Gather all active peer IDs as expected peers
        let active_peers: HashSet<String> = self
            .peers
            .iter()
            .filter(|entry| {
        ...
        proof.expected_peers = active_peers.clone();
        proof.observed_peers = active_peers.clone();

        // Build connectivity matrix from full-mesh probe results
        for entry in self.peers.iter() {
            let peer_id = entry.registration.peer_id.clone();
            let mut connected_to: HashSet<String> = HashSet::new();

            // Add peers from full-mesh probes if available
            if let Some(probes) = &entry.full_mesh_probes {
                for (target_id, result) in probes {
                    if result.reachable {
                        connected_to.insert(target_id.clone());
                    }
                }
            }

            // Also add connected peers count as approximation
            // (In production, this would be actual peer connections)
            if entry.connected_peers > 0 {
                // At minimum, we know they're connected to some peers
                // The actual list would come from the node itself
            }

            if !connected_to.is_empty() {
                proof.connectivity_matrix.insert(peer_id, connected_to);
            }
        }
    ...
    /// Validate a connectivity proof against the registry's known state.
    ///
    /// Returns validation result with details about any discrepancies.
    pub fn validate_connectivity_proof(
        &self,
        proof: &super::types::NetworkConnectivityProof,
    ) -> ProofValidationResult {
        let mut result = ProofValidationResult {
            valid: true,
            errors: Vec::new(),
            warnings: Vec::new(),
        };

        // Check 1: Verify the observer is a registered peer
        if !self.peers.contains_key(&proof.observer_id) {
            result.valid = false;
            result.errors.push(format!(
                "Observer '{}' is not a registered peer",
                proof.observer_id
            ));
        }

        // Check 2: Verify timestamp freshness (within 30 seconds)
        let age = proof.timestamp.elapsed().unwrap_or(Duration::MAX);
        if age > Duration::from_secs(30) {
            result.warnings.push(format!(
                "Proof timestamp is stale ({:.1}s old)",
                age.as_secs_f64()
            ));
        }

        // Check 3: Cross-validate observed peers against registry
        let registry_peers: std::collections::HashSet<_> = self
            .peers
            .iter()
            .map(|e| e.registration.peer_id.clone())
            .collect();
        ...
        // Check 4: Verify cross-validations
        for cv in &proof.cross_validations {
            if !cv.confirms_us {
                result.warnings.push(format!(
                    "Peer '{}' does not confirm bidirectional connection",
                    cv.peer_id
                ));
            }
        ...
        // Check 5: Connectivity matrix consistency
        for (peer, connections) in &proof.connectivity_matrix {
            for connected_peer in connections {
                // Bidirectional check: if A->B then B->A should exist
                if let Some(reverse_conns) = proof.connectivity_matrix.get(connected_peer) {
                    if !reverse_conns.contains(peer) {
                        result.warnings.push(format!(
                            "Unidirectional connection: {} -> {} but not reverse",
                            peer, connected_peer
                        ));
                    }
                }
    ...
    /// Detect false positive patterns in connectivity proofs.
    ///
    /// Analyzes multiple proofs to find inconsistencies that indicate
    /// the network isn't actually working as claimed.
    pub fn detect_false_positives(
        &self,
        proofs: &[super::types::NetworkConnectivityProof],
    ) -> Vec<super::types::TestAnomaly> {
        use super::types::TestAnomaly;

        let mut anomalies = Vec::new();

        // Pattern 1: Node A claims connection to B, but B's proof doesn't show A
        for proof_a in proofs {
            for proof_b in proofs {
                if proof_a.observer_id == proof_b.observer_id {
                    continue;
                }

                // Check if A claims to see B
                if proof_a.observed_peers.contains(&proof_b.observer_id) {
                    // B should also see A
                    if !proof_b.observed_peers.contains(&proof_a.observer_id) {
                        anomalies.push(
                            TestAnomaly::new(
                                "asymmetric_visibility".to_string(),
                                format!(
                                    "Node '{}' sees '{}' but '{}' does not see '{}'",
                                    proof_a.observer_id,
                                    proof_b.observer_id,
                                    proof_b.observer_id,
                                    proof_a.observer_id
                            ...
                            .with_nodes(vec![
                                proof_a.observer_id.clone(),
                                proof_b.observer_id.clone(),
                            ]),
                        );
                    }
                }
        ...
        // Pattern 2: Sparse connectivity despite "success" claims
        if proofs.len() >= 3 {
            let expected_connections = proofs.len() - 1; // Each should see all others
            for proof in proofs {
                let actual = proof.observed_peers.len();
                if actual < expected_connections / 2 {
                    anomalies.push(
                        TestAnomaly::new(
                            "sparse_connectivity".to_string(),
                            format!(
                                "Node '{}' only sees {}/{} expected peers",
                                proof.observer_id, actual, expected_connections
                            ),
                            3,
                        )
                        .with_nodes(vec![proof.observer_id.clone()]),
                    );
                }
            }
        }

        // Pattern 3: Cross-validation failures
        for proof in proofs {
            let failed = proof
                .cross_validations
                .iter()
            ...
            if failed > 0 {
                anomalies.push(
                    TestAnomaly::new(
                        "cross_validation_failure".to_string(),
                        format!(
                            "Node '{}' has {} failed cross-validations",
                            proof.observer_id, failed
                        ),
                        4,
                    )
                    .with_nodes(vec![proof.observer_id.clone()]),
        ...
        // Pattern 4: Matrix hash mismatches between cross-validations
        // (would indicate different views of the network)
        for proof in proofs {
            let unique_hashes: std::collections::HashSet<_> = proof
                .cross_validations
                .iter()
                .map(|cv| cv.their_matrix_hash)
                .collect();

            if unique_hashes.len() > 1 {
                anomalies.push(
                    TestAnomaly::new(
                        "divergent_network_views".to_string(),
                        format!(
                            "Node '{}' sees {} different network states from cross-validators",
                            proof.observer_id,
                            unique_hashes.len()
                        ),
                        5, // Critical severity
                    ...
                    .with_nodes(vec![proof.observer_id.clone()])
                    .with_location("crates/quic-test/src/registry/store.rs".to_string()),
                );
            }
        }
    ...
    /// Get all peer IDs that are currently active.
...
pub struct ProofValidationResult {
    ...
    /// Validation warnings (don't cause failure)
