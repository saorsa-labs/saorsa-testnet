...
// =============================================================================
// These types provide cryptographically verifiable evidence that network
// operations succeeded, enabling detection of false positives and ensuring
// hard proof of functionality.

/// Default function for SystemTime serde deserialization.
fn default_system_time() -> SystemTime {
    SystemTime::now()
}

/// Type of proof being attested.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ProofType {
    ...
    CrdtConvergence,
    /// Cross-validation of another node's proof
    CrossValidation,
}
...
impl std::fmt::Display for ProofType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::GossipProtocol => write!(f, "Gossip Protocol"),
            Self::CrdtConvergence => write!(f, "CRDT Convergence"),
            Self::CrossValidation => write!(f, "Cross-Validation"),
        }
    }
}

/// Cryptographically signed attestation of test results.
///
/// Uses BLAKE3 for payload hashing and ML-DSA-65 (FIPS 204) post-quantum
/// signatures for attestation signing. Each node signs its proofs with its
/// identity key, enabling cross-validation of claims.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SignedAttestation {
    ...
    /// BLAKE3 hash of the proof payload
    pub payload_hash: [u8; 32],
    /// ML-DSA-65 signature over (node_id || timestamp || proof_type || payload_hash)
    pub signature: Vec<u8>,
}

impl SignedAttestation {
    /// Create a new attestation (signature must be added separately).
    pub fn new(node_id: String, proof_type: ProofType, payload_hash: [u8; 32]) -> Self {
        Self {
            node_id,
    ...
    /// Check if the attestation timestamp is fresh (within threshold).
...
impl Default for SignedAttestation {
    fn default() -> Self {
        Self {
            ...
            proof_type: ProofType::Connectivity,
            payload_hash: [0u8; 32],
            signature: Vec::new(),
        }
    }
...
/// Cross-validation of connectivity claims between peers.
///
/// To detect false positives, each node's connectivity claim must be
/// verified by the peer it claims to be connected to.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CrossValidation {
    /// The peer being asked to validate
    pub peer_id: String,
    /// Whether the peer confirms they can see us
    pub confirms_us: bool,
    /// BLAKE3 hash of their connectivity matrix
    pub their_matrix_hash: [u8; 32],
    /// Their signed attestation of this validation
    pub attestation: SignedAttestation,
}

impl CrossValidation {
    /// Create a new cross-validation result.
    pub fn new(peer_id: String, confirms_us: bool, their_matrix_hash: [u8; 32]) -> Self {
        Self {
            attestation: SignedAttestation::new(
                peer_id.clone(),
                ProofType::CrossValidation,
                their_matrix_hash,
            ),
            peer_id,
            confirms_us,
...
/// Full connectivity proof with cross-validation.
///
/// This proof demonstrates that a node can see other nodes on the network
/// AND that those nodes confirm the connection is bidirectional. Without
/// cross-validation, a node could falsely report success.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetworkConnectivityProof {
    /// The node making this proof
    pub observer_id: String,
    /// When this proof was generated
    #[serde(default = "default_system_time")]
    pub timestamp: SystemTime,
    /// Peers we expected to see (from registry)
    #[serde(default)]
    pub expected_peers: HashSet<String>,
    /// Peers we actually observed
    #[serde(default)]
    pub observed_peers: HashSet<String>,
    /// Full connectivity matrix - who can reach whom
    #[serde(default)]
    pub connectivity_matrix: HashMap<String, HashSet<String>>,
    /// Cross-validations from each observed peer
    #[serde(default)]
    pub cross_validations: Vec<CrossValidation>,
    /// Signed attestation of this proof
    #[serde(default)]
    pub attestation: SignedAttestation,
}

impl Default for NetworkConnectivityProof {
    fn default() -> Self {
        Self {
            ...
            attestation: SignedAttestation::default(),
        }
    }
}

impl NetworkConnectivityProof {
    /// Create a new connectivity proof for an observer.
    ...
    /// Check if all expected peers were observed.
    pub fn all_peers_visible(&self) -> bool {
        self.expected_peers.is_subset(&self.observed_peers)
    }

    /// Get the number of missing peers.
    pub fn missing_peer_count(&self) -> usize {
        self.expected_peers.difference(&self.observed_peers).count()
    }

    /// Get the list of missing peers.
    pub fn missing_peers(&self) -> Vec<&String> {
        self.expected_peers
            .difference(&self.observed_peers)
            .collect()
    }
    ...
    pub fn failed_validations(&self) -> Vec<&CrossValidation> {
    ...
    /// Calculate the cross-validation success rate.
    pub fn cross_validation_rate(&self) -> f64 {
    ...
    /// Check if this proof is valid (all criteria met).
    pub fn is_valid(&self) -> bool {
        self.all_peers_visible() && self.all_cross_validated()
    }
}

/// HyParView gossip overlay proof.
///
/// Verifies the HyParView epidemic protocol is functioning correctly:
/// - Active view maintained at configured size
/// - Passive view populated for failover
/// - Shuffle mechanism propagating view updates
/// - Bidirectional connections (A sees B implies B sees A)
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct HyParViewProof {
    /// Size of active view (should match configuration)
    pub active_view_size: usize,
    /// Size of passive view
    pub passive_view_size: usize,
    /// Shuffle success rate (0.0-1.0)
    pub shuffle_success_rate: f64,
    /// Time for views to converge after node join (ms)
    pub view_convergence_time_ms: u64,
    /// Verified bidirectional connections (both ends confirmed)
    pub bidirectional_connections: Vec<(String, String)>,
    /// Expected active view size from config
    pub expected_active_size: usize,
    /// Expected passive view size from config
    pub expected_passive_size: usize,
}

impl HyParViewProof {
    /// Check if active view size is acceptable.
    ///
    /// For small testnets (< 10 nodes), we accept any active view >= 1.
    /// For larger networks, we require at least 50% of expected size.
    pub fn active_view_correct(&self) -> bool {
        if self.expected_active_size <= 4 {
            // For small networks, any active peers is acceptable
            self.active_view_size >= 1
        } else {
            // For larger networks, require at least 50% of expected
            self.active_view_size >= self.expected_active_size / 2
        }
    }

    /// Check if all connections are bidirectional.
    pub fn all_bidirectional(&self) -> bool {
        !self.bidirectional_connections.is_empty()
    }

    /// Check if shuffle mechanism is working (>80% success).
    ///
    /// If no shuffles have occurred yet, we skip this check.
    pub fn shuffle_working(&self) -> bool {
        if self.shuffle_success_rate == 0.0 && self.view_convergence_time_ms == 0 {
            // No shuffle data yet - skip this check for new networks
            true
        } else {
            self.shuffle_success_rate >= 0.8
        }
    }

    /// Check if this proof passes all criteria.
    pub fn is_valid(&self) -> bool {
        self.active_view_correct() && self.all_bidirectional() && self.shuffle_working()
    }
}

/// SWIM failure detection proof.
///
/// Verifies the SWIM protocol is detecting failures correctly:
/// - Ping probes succeeding for live nodes
/// - Ping-req indirect probes working when direct fails
/// - Failure detection happening within expected latency
/// - False positive rate below threshold
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct SwimProof {
    /// Direct ping success rate (0.0-1.0)
    pub ping_success_rate: f64,
    /// Indirect ping-req success rate (0.0-1.0)
    pub ping_req_success_rate: f64,
    /// Time to detect simulated failure (ms)
    pub failure_detection_latency_ms: u64,
    /// Rate of live nodes incorrectly marked dead (0.0-1.0)
    pub false_positive_rate: f64,
    /// Whether protocol period is consistent
    pub protocol_period_consistent: bool,
    /// Number of probes sent during test
    pub probes_sent: u64,
    /// Number of probes that received response
    pub probes_received: u64,
}

impl SwimProof {
    /// Check if ping success rate is acceptable (>90%).
    ///
    /// If no probes have been sent yet, we skip this check.
    pub fn ping_acceptable(&self) -> bool {
        if self.probes_sent == 0 {
            // No probes tracked - skip ping rate check
            true
        } else {
            self.ping_success_rate >= 0.9
        }
    }

    /// Check if false positive rate is acceptable (<5%).
    pub fn false_positives_acceptable(&self) -> bool {
        self.false_positive_rate <= 0.05
    }

    /// Check if this proof passes all criteria.
    pub fn is_valid(&self) -> bool {
        self.ping_acceptable()
            && self.false_positives_acceptable()
            && self.protocol_period_consistent
    }
}

/// Plumtree broadcast tree proof.
///
/// Verifies the Plumtree epidemic broadcast is functioning:
/// - Eager push delivering messages to immediate neighbors
/// - Lazy push recovering missed messages via IHAVE/GRAFT
/// - Message delivery latency within bounds
/// - Tree structure is valid (no cycles, all nodes reachable)
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct PlumtreeProof {
    /// Eager push message delivery rate (0.0-1.0)
    pub eager_push_delivery_rate: f64,
    /// Lazy push recovery success rate (0.0-1.0)
    pub lazy_push_recovery_rate: f64,
    /// IHAVE/GRAFT exchange success rate (0.0-1.0)
    pub ihave_graft_success_rate: f64,
    /// Average message delivery latency (ms)
    pub message_delivery_latency_ms: u64,
    /// Whether tree structure is valid
    pub tree_structure_valid: bool,
    /// Total messages broadcast during test
...
impl PlumtreeProof {
    /// Check if message delivery is acceptable (>85%).
    ///
    /// For small networks with limited traffic, we accept 85% delivery.
    /// This accounts for network latency, transient failures, and
    /// nodes joining/leaving during tests.
    pub fn delivery_acceptable(&self) -> bool {
        if self.messages_broadcast == 0 {
            return true;
        }
        let rate = self.messages_delivered as f64 / self.messages_broadcast as f64;
        rate >= 0.85
    }

    /// Check if this proof passes all criteria.
    ///
    /// For networks with no broadcast activity yet, we only check delivery rate.
    pub fn is_valid(&self) -> bool {
        if self.messages_broadcast == 0 {
            // No messages broadcast - skip tree structure check
            true
        } else {
            self.delivery_acceptable() && self.tree_structure_valid
        }
    }
}

/// Combined gossip protocol proof.
///
/// Aggregates proofs from all gossip protocols (HyParView, SWIM, Plumtree)
/// into a single verifiable structure.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GossipProtocolProof {
    /// HyParView overlay verification
    #[serde(default)]
    pub hyparview: HyParViewProof,
    /// SWIM failure detection verification
    #[serde(default)]
    pub swim: SwimProof,
    /// Plumtree broadcast verification
    #[serde(default)]
    pub plumtree: PlumtreeProof,
    /// Signed attestation of this proof
    #[serde(default)]
    pub attestation: SignedAttestation,
    /// When this proof was generated
...
impl Default for GossipProtocolProof {
    fn default() -> Self {
        Self {
            hyparview: HyParViewProof::default(),
            swim: SwimProof::default(),
            plumtree: PlumtreeProof::default(),
            attestation: SignedAttestation::default(),
            timestamp: SystemTime::now(),
        }
    }
...
impl GossipProtocolProof {
    ...
    /// Check if all protocols pass verification.
    pub fn all_protocols_valid(&self) -> bool {
        self.hyparview.is_valid() && self.swim.is_valid() && self.plumtree.is_valid()
    }
}

/// CRDT type being tested.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum CrdtType {
...
impl std::fmt::Display for CrdtType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::OrSet => write!(f, "OR-Set"),
            Self::GCounter => write!(f, "G-Counter"),
            Self::PnCounter => write!(f, "PN-Counter"),
            Self::LwwRegister => write!(f, "LWW-Register"),
            Self::PeerCache => write!(f, "PeerCache"),
        }
    }
}

/// A single CRDT operation recorded during testing.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CrdtOperation {
    /// Node that performed the operation
    pub node_id: String,
    /// Type of operation (add, remove, increment, etc.)
    pub operation_type: String,
    /// Vector clock at time of operation
    pub vector_clock: HashMap<String, u64>,
    /// When the operation was performed
    pub timestamp: SystemTime,
    /// BLAKE3 hash of the operation payload
    pub payload_hash: [u8; 32],
}

impl CrdtOperation {
    /// Create a new CRDT operation record.
    pub fn new(node_id: String, operation_type: String) -> Self {
        Self {
            node_id,
            operation_type,
...
///
/// Proves that after concurrent updates from multiple nodes, all nodes
/// converge to the same final state. This is the key property of CRDTs.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CrdtConvergenceProof {
    /// Unique test identifier
    pub test_id: String,
    /// Type of CRDT being tested
    #[serde(default)]
    pub crdt_type: CrdtType,
    /// BLAKE3 hash of initial state before test
    #[serde(default)]
    pub initial_state_hash: [u8; 32],
    /// All operations performed during test
    #[serde(default)]
    pub operations: Vec<CrdtOperation>,
    /// Final state hash from each node
    #[serde(default)]
    pub node_final_states: HashMap<String, [u8; 32]>,
    /// Whether all nodes converged to same state
    #[serde(default)]
    pub convergence_achieved: bool,
    /// Time to achieve convergence (ms)
    #[serde(default)]
    pub convergence_time_ms: u64,
    /// Whether conflict resolution was correct
    #[serde(default)]
    pub conflict_resolution_correct: bool,
    /// Signed attestations from each participating node
    #[serde(default)]
...
impl Default for CrdtConvergenceProof {
    fn default() -> Self {
        Self {
            test_id: String::new(),
...
impl CrdtConvergenceProof {
    /// Create a new CRDT convergence proof.
    pub fn new(test_id: String, crdt_type: CrdtType) -> Self {
        Self {
            test_id,
            crdt_type,
            timestamp: SystemTime::now(),
    ...
    /// Check if all nodes have the same final state.
    pub fn states_match(&self) -> bool {
        if self.node_final_states.len() <= 1 {
            return true;
        }
        let mut hashes = self.node_final_states.values();
        let first = hashes.next();
        first.map(|f| hashes.all(|h| h == f)).unwrap_or(true)
    }
    ...
    pub fn divergent_nodes(&self) -> Vec<&String> {
        ...
        for hash in self.node_final_states.values() {
    ...
    pub fn is_valid(&self) -> bool {
        self.convergence_achieved && self.states_match() && self.conflict_resolution_correct
    }

    /// Get the number of participating nodes.
    pub fn node_count(&self) -> usize {
        self.node_final_states.len()
    }
...
pub struct TestAnomaly {
    ...
    /// Suggested code location to investigate
    pub suggested_location: Option<String>,
}

impl TestAnomaly {
    /// Create a new anomaly record.
    pub fn new(anomaly_type: String, description: String, severity: u8) -> Self {
        Self {
            anomaly_type,
            description,
            severity,
    ...
    /// Add nodes involved in this anomaly.
    pub fn with_nodes(mut self, nodes: Vec<String>) -> Self {
        self.nodes_involved = nodes;
        self
    }

    /// Add suggested code location.
    pub fn with_location(mut self, location: String) -> Self {
        self.suggested_location = Some(location);
        self
    }
}

/// Comprehensive test report with all proofs.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProofBasedTestReport {
    ...
    #[serde(default)]
    pub connectivity: Option<NetworkConnectivityProof>,
    /// Gossip protocol proof
    #[serde(default)]
    pub gossip: Option<GossipProtocolProof>,
    /// CRDT convergence proof
    #[serde(default)]
    pub crdt: Option<CrdtConvergenceProof>,
    /// Anomalies detected during testing
    #[serde(default)]
...
impl Default for ProofBasedTestReport {
    fn default() -> Self {
        Self {
...
impl ProofBasedTestReport {
    ...
    pub fn complete(&mut self) {
        ...
        self.passed = self.all_proofs_valid();
    }
    ...
    pub fn add_anomaly(&mut self, anomaly: TestAnomaly) {
        self.anomalies.push(anomaly);
    }

    /// Get high-severity anomalies (severity >= 4).
    pub fn critical_anomalies(&self) -> Vec<&TestAnomaly> {
        self.anomalies.iter().filter(|a| a.severity >= 4).collect()
    }
