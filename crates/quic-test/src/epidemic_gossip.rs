...
impl GroupState {
    ...
    pub fn join_group(&mut self, entity_name: &str) -> TopicId {
...
impl EpidemicGossip {
    ...
    pub async fn start(&self) -> Result<(), GossipError> {
        ...
        // Initialize default groups for topic subscriptions
        {
            ...
            for group_name in DEFAULT_GROUPS {
                let topic_id = groups_guard.join_group(group_name);
                info!(
    ...
    /// Get current gossip statistics without blocking.
    ///
    /// This method uses try_read() to avoid blocking if the stats lock is held.
    /// Returns None if the lock cannot be acquired immediately, allowing callers
    /// to use cached/default values instead of blocking heartbeat loops.
    pub fn stats_nonblocking(&self) -> Option<GossipStats> {
        self.stats.try_read().ok().map(|guard| guard.clone())
    }
    ...
    pub async fn join_group(&self, entity_name: &str) -> TopicId {
        let mut groups_guard = self.groups_state.write().await;
        groups_guard.join_group(entity_name)
    }
    ...
    fn spawn_stats_updater(&self) {
        ...
        tokio::spawn(async move {
            // Use 7-second interval to avoid lock contention with heartbeat (5s interval)
            // This prevents the stats write lock from blocking heartbeat reads
...
mod tests {
    ...
    fn test_group_state_join_group() {
        ...
        let topic_id = state.join_group("test-group");
    ...
    fn test_group_state_leave_nonexistent_group() {
        let mut state = GroupState::new();
        let fake_topic = TopicId::from_entity("fake");
    ...
    fn test_group_state_stats() {
        ...
        state.join_group("group1");
        state.join_group("group2");
    ...
    async fn test_epidemic_gossip_join_group() {
        ...
        let topic_id = gossip.join_group("custom-group").await;
    ...
    async fn test_epidemic_gossip_get_groups() {
        ...
        let topic1 = gossip.join_group("group-a").await;
        let topic2 = gossip.join_group("group-b").await;
    ...
    async fn test_epidemic_gossip_group_epoch() {
        ...
        let topic_id = gossip.join_group("test-group").await;
    ...
    async fn test_epidemic_gossip_group_publish_not_member() {
        ...
        let fake_topic = TopicId::from_entity("not-joined");
