...
fn draw_tab_bar(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let tabs = Tabs::new(tab_titles)
        ...
        .highlight_style(
            Style::default()
                // Use Green for selected tab - visible on both light and dark backgrounds
                .fg(Color::Green)
...
fn draw_overview_tab(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    draw_proof_status(frame, app, chunks[0]);
...
/// Draw the proof verification status panel.
fn draw_proof_status(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let line1 = Line::from(vec![
        ...
        Span::styled(
            format!(
                "{}/{} nodes",
                status.connectivity_nodes.0, status.connectivity_nodes.1
            ),
            Style::default().fg(Color::Cyan),
        ),
        Span::raw("    Gossip: "),
        Span::styled(gossip_icon, Style::default().fg(gossip_color)),
        Span::raw(" "),
        Span::styled(
            if status.gossip_pass { "PASS" } else { "FAIL" },
            Style::default().fg(gossip_color),
        ),
        Span::raw("  "),
        Span::styled(
            format!(
                "HyPar:{} SWIM:{} Tree:{}",
                status.hyparview_active,
                status.swim_alive,
                if status.tree_valid { "ok" } else { "bad" }
            ),
            Style::default().fg(Color::Cyan),
        ),
    ]);
    ...
    let line2 = Line::from(vec![
        Span::raw("  CRDT:         "),
        Span::styled(crdt_icon, Style::default().fg(crdt_color)),
        Span::raw(" "),
        Span::styled(
            if status.crdt_pass { "PASS" } else { "FAIL" },
            Style::default().fg(crdt_color),
        ),
        Span::raw("  "),
        Span::styled(
            format!("{} nodes (hash: {})", status.crdt_nodes, crdt_hash),
            Style::default().fg(Color::Cyan),
        ),
        Span::raw("    NAT:    "),
        Span::styled(nat_icon, Style::default().fg(nat_color)),
        Span::raw(" "),
        Span::styled(
            if status.nat_pass { "PASS" } else { "FAIL" },
            Style::default().fg(nat_color),
        ),
        Span::raw("  "),
        Span::styled(
            format!(
                "D:{} P:{} R:{}",
                status.nat_direct, status.nat_punched, status.nat_relayed
            ),
            Style::default().fg(Color::Cyan),
        ),
    ]);
...
fn draw_node_info(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let line1 = Line::from(vec![
        Span::raw("  ID: "),
        Span::styled(peer_id_display, Style::default().fg(Color::Cyan)),
        Span::raw("  NAT: "),
